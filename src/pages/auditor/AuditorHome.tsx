...
function getEffectiveWorkflowStatus(item: InstallationItem): NonNullable<InstallationItem["workflowStatus"]> {
  const itemDayKey = item.dayKey ?? "";

  if (!item.workflowStatus && itemDayKey && itemDayKey <= WORKFLOW_CUTOFF_DAY) {
    return "SOLUCIONADO";
  }
...
useEffect(() => {
  ...
  const squadStats = useMemo(() => {
    ...
    for (const sid of Object.keys(groups)) {
      ...
      list.forEach((it) => {
        const effectiveStatus = getEffectiveWorkflowStatus(it);

        if (effectiveStatus === "SOLUCIONADO") solvedCount += 1;
        else if (effectiveStatus === "NO_SOLUCIONADO") notSolvedCount += 1;
        else if (effectiveStatus === "RECHAZADO") rejectedCount += 1;
        else pendingCount += 1;
      });
  ...
  const totalSolvedInstallations = useMemo(
    () =>
      installations.filter((it) => getEffectiveWorkflowStatus(it) === "SOLUCIONADO").length,
    [installations],
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 order-1 sm:order-5">
        ...
        <div className="rounded-2xl border p-3 sm:p-4">
          ...
          <div className="text-[11px] text-gray-500">
            Histórico sin estado hasta {WORKFLOW_CUTOFF_DAY}: se cuenta como solucionado.
          </div>
        </div>
        <div className="rounded-2xl border p-3 sm:p-4">
              ...
              return (
                ...
                >
                  ...
                  <div className={active ? "text-white text-right" : "text-right"}>
                    <div className="font-bold text-sm">Hechas: {st.solvedCount}</div>
                    <div className={active ? "text-white/70 text-[11px]" : "text-gray-500 text-[11px]"}>
                      No sol: {st.notSolvedCount} · Rech: {st.rejectedCount}
                    </div>
                    <div className={active ? "text-white/70 text-[11px]" : "text-gray-500 text-[11px]"}>
                      Pend: {st.pendingCount}
                    </div>
        ...
        <div className="rounded-2xl border p-3 sm:p-4 lg:col-span-2">
          <div className="flex items-end justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500">
                Hechas hoy: <span className="font-medium">{selectedStatusStats.SOLUCIONADO}</span>{" "}
                · Pendientes: <span className="font-medium">{selectedStatusStats.PENDIENTE}</span>
              </p>
            </div>
          </div>
          ...
          {selectedSquadId && filteredSelectedItems.length === 0 ? (
            ...
            <ul className="space-y-1.5 sm:space-y-2">
              {filteredSelectedItems.map(({ it, diffMin, gapText }) => {
                let alertLabel = "";
                let alertClass = "";
                const statusMeta = getWorkflowStatusMeta(getEffectiveWorkflowStatus(it));
