...
const searchResult = useMemo(() => {
  ...
  return installations.find(
    (it) => it.idInstalacion?.toLowerCase() === q
  ) ?? null;
...
console.log("SQUADS RAW:", snap.docs.map(d => ({ id: d.id, ...d.data() })));
        // orden Q01..Q99
...
useEffect(() => {
  const q = query(collection(db, "installations"), where("dayKey", "==", dayKey));
  const unsub = onSnapshot(
    q,
    (snap) => {
      const rows = snap.docs.map((d) => ({
  ...
  {searchId && (
    <div className="mt-3">
      {searchResult ? (
        <div className="border rounded-xl p-3 bg-green-50">
          ...
          <div className="text-sm text-gray-700">
            ...
            <span className="font-medium">
              {squads.find(s => s.id === searchResult.squadId)?.name ?? searchResult.squadId}
            </span>
          </div>
        ...
        <div>
          <h1 className="text-2xl font-bold">{profile?.displayName ?? "Auditor"}</h1>
    ...
    const st = squadStats[s.id] ?? { count: 0, lastAt: null, avgGapMin: 0, maxGapMin: 0 };
              ...
              return (
                <button
                  ...
                  className={[
                    "w-full text-left border rounded-xl p-3 flex items-center justify-between",
                    active ? "bg-black text-white border-black" : "bg-white",
                  ].join(" ")}
                >
                  <div>
                    <div className="font-medium">{s.name}</div>
                    <div className={active ? "text-white/70 text-xs" : "text-gray-500 text-xs"}>
                      ID: {s.id}
                    </div>
                  </div>

<div className={active ? "text-white text-right" : "text-right"}>
  <div className={active ? "text-white font-bold" : "font-bold"}>{st.count}</div>
  <div className={active ? "text-white/70 text-xs" : "text-gray-500 text-xs"}>
    ...
    últ: {st.lastAt ? fmtTime(st.lastAt) : "-"}
        ...
        <div className="rounded-2xl border p-4 lg:col-span-2">
          <div className="flex items-end justify-between gap-3 mb-3">
            <div>
              <h2 className="font-semibold">{selectedSquadName || "Seleccioná una cuadrilla"}</h2>
              <p className="text-sm text-gray-500">
                Instalaciones: <span className="font-medium">{selectedList.length}</span>
              </p>
            </div>
          </div>

          {selectedSquadId && selectedList.length === 0 ? (
            <p className="text-sm text-gray-500">No hay instalaciones para esta cuadrilla en la fecha elegida.</p>
          ) : (
